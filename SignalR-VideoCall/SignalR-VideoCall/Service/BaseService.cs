...
namespace SignalRVideoCall.Service
{
    public class BaseService
    {
        ...
        protected async Task<HttpClient> GetClient(string baseUrl, bool withoutToken = false)
        {
            ...
            var user = await NativeOperation.SessionService.GetConnectedUser();
            var token = await NativeOperation.SessionService.GetToken();
            ...
            try
            {
                if (user != null)
                {
                    if (token.TokenExpireTime.AddSeconds(30) <= DateTime.UtcNow)
                    {
                        ...
                        relase = true;
                        user = await NativeOperation.SessionService.GetConnectedUser();
                        ...
                        if (token.TokenExpireTime.AddSeconds(30) <= DateTime.UtcNow)
                        {
                            ...
                            {
                                await NativeOperation.SessionService.SetConnectedUser(null);
                                await NativeOperation.SessionService.SetToken(null);
